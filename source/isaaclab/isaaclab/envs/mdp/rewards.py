...
def action_rate_l2_clipped(env: ManagerBasedRLEnv) -> torch.Tensor:
    """Penalize the rate of change of the actions using L2 squared kernel with clipping to prevent training instability."""
    action_diff = torch.sum(torch.square(env.action_manager.action - env.action_manager.prev_action), dim=1)
    return torch.clamp(action_diff, min=0.0, max=1.0)
